...
words unchanged (5 s / 5 s / 3, 9.9 bar reported peak). The owner confirmed all
three physical repetitions and normal completion for this run. See
...
| A09 | Cleaning start/stop | Impl: coil 155 pulse, same action for both directions with state guards [G-control] | One attended start observed cleaning then idle, all settings unchanged; owner confirmed three repetitions and normal completion; stop untested and not exposed | Opt-in attended start only |
