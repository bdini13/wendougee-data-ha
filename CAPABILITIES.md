...
Our Python package implements CRC, allowlisted read requests, strict response framing, ten-field telemetry and configuration/state decoders, serialized failure-quarantined read sessions, and a one-shot Bleak CLI. These have offline tests, including fake Bluetooth lifecycle failures; see [offline implementation details](docs/OFFLINE_CORE.md). One native CoreBluetooth helper obtained a valid idle telemetry response; that does **not** validate the Python transport. The [read-only HA integration](docs/HOME_ASSISTANT.md) defines 23 read-only entities, split runtime/configuration polling, failure recovery and redacted diagnostics, tested with simulated Bluetooth under HA 2026.9.3. Version 0.0.8 is also installed on HA 2026.9.1 and completed restart, scheduled split-cadence polling and config-entry reload checks through the ESPHome proxy; see the [sanitized record](docs/LIVE_VALIDATION_2026-09-21.md). No control is implemented here.
...
| T06 | Request/session handling | Impl upstream [G-session]; local strict stream/session with one outstanding request | HA restart, >10-minute split-cadence window and config-entry reload succeeded; deliberate disconnect/contention and long soak pending | Unavailable/reconnect handling |
