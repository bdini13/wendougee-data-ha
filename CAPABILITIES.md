...
Our Python package implements CRC, allowlisted read requests, strict response framing, ten-field telemetry and configuration/state decoders, serialized failure-quarantined read sessions, and a one-shot Bleak CLI. These have offline tests, including fake Bluetooth lifecycle failures; see [offline implementation details](docs/OFFLINE_CORE.md). The first installed 0.1.1 benchmark invocation exposed an implementation timing/timeout defect before any rate was accepted. Version 0.1.2 fixes that defect with a 1 → 2 → 5 → 10 Hz ladder and is installed on HA 2026.9.1 after a fresh backup, checksum verification, configuration check and healthy restart. The corrected benchmark selected 2 Hz for complete telemetry/state pairs; its 5 Hz stage sustained about 2.46 valid pairs/s and failed only the cadence threshold. On 2026-09-23 the same ESP32 path stopped opening an active BLE connection even though the exact target advertised, slots were free and the official app could connect; bounded debug attempts failed before GATT or Modbus. A controlled 2026-09-26 A/B test reproduced the timeout on a second original ESP32 running ESPHome 2026.9.0. With mobile Bluetooth excluded, one later attempt reached `Connection open` and late service discovery, but neither proxy has produced a repeatable completed session; `GATTC_ConfigureMTU GATT_BUSY` remains part of the cleanup path. The earlier rate is therefore historical evidence, not a currently available sampling guarantee. The private bounded dynamic trace has not run on hardware. An offline-only module constructs the four documented boiler FC06 request shapes and validates exact echoes, but no Home Assistant service, control entity or transport can send them. See the [HA guide](docs/HOME_ASSISTANT.md), [control design](docs/CONTROL_DESIGN.md) and [sanitized live record](docs/LIVE_VALIDATION_2026-09-21.md).
...
| T06 | Request/session handling | Impl upstream [G-session]; local strict stream/session with one outstanding request | Earlier HA restart, >10-minute split-cadence window and config-entry reload succeeded; on 2026-09-23 the ESP32 stopped before `Connection open` despite clean advertisement/slot evidence, so GATT and Modbus were not reached | Unavailable/reconnect handling |
...
| T08 | Pairing/security and app coexistence | Notes: direct access without pairing [L]; no pairing prompt in local native read | Official app connected during the 2026-09-23 failure investigation, while the ESP32 could not open a link after app release; cause and coexistence policy remain unresolved | Single-owner connection policy |
