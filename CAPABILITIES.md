...
Our Python package implements CRC, allowlisted read requests, strict response framing, ten-field telemetry and configuration/state decoders, serialized failure-quarantined read sessions, and a one-shot Bleak CLI. These have offline tests, including fake Bluetooth lifecycle failures; see [offline implementation details](docs/OFFLINE_CORE.md). Version 0.1.1 is installed on HA 2026.9.1 after backup, configuration check and a healthy restart; 0.1.0 remains the latest fully authenticated 30-entity/runtime checkpoint. The new fail-closed 5 Hz → 10 Hz benchmark and private bounded dynamic trace have not run on hardware yet. An offline-only module constructs the four documented boiler FC06 request shapes and validates exact echoes, but no Home Assistant service, control entity or transport can send them. See the [HA guide](docs/HOME_ASSISTANT.md), [control design](docs/CONTROL_DESIGN.md) and [sanitized live record](docs/LIVE_VALIDATION_2026-09-21.md).
