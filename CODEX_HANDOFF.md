...
**2026-09-23 active-connection investigation:** after the prior successful benchmark, the next benchmark stopped at 1 Hz with zero samples and a transport/protocol failure, and entry reload failed. Restarts and machine/proxy power cycles did not restore reads. The official app could still connect. A later raw proxy observation saw the exact configured public-address target at -69 dBm mean RSSI while the ESPHome API was healthy and all three active slots were free. In that clean window, a single V3 connection-only attempt discovered the target and allocated a slot but never reached `Connection open`; it timed out before GATT discovery, subscriptions or Modbus. Cached V3 had the same boundary and legacy V1 is removed. The proxy reports original ESP32 hardware and ESPHome 2026.9.0. Upstream issue 18614 is related but not an exact match: its connection opened and failed later, and its fix (PR 18609) shipped by 2026.9.0. The next gate is now a controlled second-central or alternate-proxy A/B test. Do not flash/pin proxy firmware under read-only authorization; treat that as a separately approved rollback-safe maintenance experiment. No machine control or configuration write was sent.
