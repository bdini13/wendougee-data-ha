...
**HA implementation update:** `docs/HOME_ASSISTANT.md` describes the read-only integration and build. It uses shared Bluetooth, confirmed setup, one telemetry read per short polling session, nine measurement sensors, alarm/reachability sensors, failure recovery, cancellation cleanup and allowlisted diagnostics. Normal polling remains telemetry-only. An explicitly confirmed, response-only HA action can perform the four fixed baseline reads once through a shared adapter; it has no arbitrary request or control path. A headless YAML opt-in handles both cached and later Bluetooth discovery and fails closed rather than selecting among multiple cached machines. Its optional one-shot capture flag creates an attempt marker before the four-read session, writes an owner-only private file on success, and never retries after failure or restart. Verification: 90 independent protocol/package tests on Python 3.13 and 26 real HA framework tests on HA 2026.9.3 / Python 3.14, with simulated Bluetooth and network disabled. Build `scripts/build_integration.py` before HA tests; it generates the ignored `_protocol` copy from our own sources and `dist/wendougee_data.zip`. The versioned private evidence envelope and approval-gated baseline paths are documented in `docs/EVIDENCE_COLLECTION.md`; neither four-read path has been run against the machine. On 2026-09-20, a full target-HA backup was created and the package was installed/configuration-validated on HA 2026.9.1. The registered ESPHome proxy uses the official active generic proxy package, but HA saw no matching machine advertisement during bounded waits, so no config entry or request was created. Live validation remains pending.
