...
**HA implementation update:** `docs/HOME_ASSISTANT.md` describes the read-only integration and build. Version 0.0.8 uses shared Bluetooth, confirmed setup, 23 read-only entities, split runtime/configuration polling, failure recovery, cancellation cleanup and allowlisted diagnostics. Normal polling reads telemetry plus operating state; every twentieth poll refreshes the complete fixed configuration set. An explicitly confirmed, response-only HA action can return the four fixed baseline responses; it has no arbitrary request or control path. A headless YAML opt-in handles both cached and later Bluetooth discovery and fails closed rather than selecting among multiple cached machines. Its optional one-shot capture flag creates an attempt marker before the first complete refresh, writes an owner-only private file on success, and never repeats the raw capture after failure or restart. Verification: 90 independent protocol/package tests on Python 3.13 and 29 real HA framework tests on HA 2026.9.3 / Python 3.14, with simulated Bluetooth and network disabled. Build `scripts/build_integration.py` before HA tests; it generates the ignored `_protocol` copy from our own sources and `dist/wendougee_data.zip`. On 2026-09-20, a full target-HA backup was created and package 0.0.7 was installed/configuration-validated on HA 2026.9.1. On 2026-09-21, after repairing the proxy entry's missing encryption credential from its existing ESPHome configuration, HA discovered the machine, completed all four fixed reads once, and registered that version's 11 entities. The owner-only capture remains private; only reviewed decoded values are documented in `docs/LIVE_VALIDATION_2026-09-21.md`. Version 0.0.8 remains pending live deployment; physical comparison, reconnect/soak testing and direct-Python validation remain pending.
