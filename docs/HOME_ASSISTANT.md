...
On this installation, the corrected benchmark accepted 1 Hz and 2 Hz, then rejected 5 Hz for insufficient cadence after sustaining about 2.46 complete pairs/s with roughly 410 ms mean pair round-trip time. The selected 2 Hz is a validated rate for the current **two-read pair path through this proxy**, not proof that the machine cannot return telemetry alone more frequently.
