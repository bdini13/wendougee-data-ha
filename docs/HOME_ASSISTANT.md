...
On this installation, the corrected HA benchmark accepted 1 Hz and 2 Hz, then rejected 5 Hz for insufficient cadence after sustaining about 2.46 complete pairs/s with roughly 410 ms mean pair round-trip time. This remains the selected rate for the **two-read pair path through this proxy**. A separate direct-proxy telemetry-only ladder later accepted 8 Hz (8.10 reads/s, 122 ms mean round trip) and rejected 10 Hz after sustaining 8.01 reads/s. That telemetry-only result does not change the integration's paired-sample selection until the HA benchmark is rerun.
