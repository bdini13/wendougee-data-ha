...
No high-rate polling cadence is locally validated yet. The first installed 0.1.1 benchmark returned no accepted rate because its measurement window included proxy connection setup and its session timeout was too short; this is an implementation defect, not evidence of a machine limit. Version 0.1.2 fixes those issues offline, tries 1, 2, 5 and then 10 Hz, requires at least 85% sustained cadence, stops at the first failed stage, returns a privacy-safe failure category and keeps the selected rate only in memory. Deploy it only after backup and use it before the first bounded dynamic trace. Do not scan arbitrary address ranges.
