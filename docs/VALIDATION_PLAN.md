...
- Add fake-transport tests for notification setup failure, disconnect while awaiting response, cleanup failures, duplicate frames, delayed replies, reconnect and cancellation. These now cover failure quarantine and fresh-session recovery. The HA coordinator opens a fresh short telemetry session for each poll, including recovery after failures; a persistent connection manager is not implemented.
