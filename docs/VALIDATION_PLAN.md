...
The corrected 0.1.2 benchmark selected 2 Hz for complete telemetry/state pairs through this ESPHome proxy. Its 5 Hz stage returned valid pairs at about 2.46 Hz but missed the 85% cadence threshold without a transport/protocol failure. This validates 2 Hz for the current two-read path, not the machine's telemetry-only maximum. The selected rate is kept only in memory and must be revalidated after reload/restart. Use it before the first bounded dynamic trace. Do not scan arbitrary address ranges.
